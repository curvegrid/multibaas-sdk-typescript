...
export class APIError extends Error {
  ...
  constructor(originalError: AxiosError<ErrorResponse>) {
    const { status, message } = originalError.response?.data || { status: 0, message: 'Unknown error' };
    super(message);
    this.name = 'APIError';
...
(async () => {
  console.log('CommonJS Example: Starting MultiBaas SDK test...');
  ...
  try {
    ...
    if (isAxiosError(e)) {
      console.log(
        `Example 3: The callContractFunction method correctly threw an error: ${
  ...
  console.log('CommonJS Example: All tests completed successfully!');
})();
